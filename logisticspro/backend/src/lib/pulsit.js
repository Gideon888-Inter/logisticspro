 ...
 *
 * GET /api/Vehicles is the data source — it carries live position
 * (latitude/longitude), speed, heading, odometer, ignition status,
 * vehicleType, registrationNumber, and a vehicleDescription field that
 * matches our internal fleet codes (MH173, MH191, ST83, etc).
 *
 * AUTH: POST /Auth/login with { apiKey } returns a long-lived bearer
 ...
 * many LP2.0 callers (tracking.js, vehicles.js fleet-overview, etc.) are
 * requesting — only one upstream call per cache window, shared across
...
// ── Cached vehicle list — single upstream call shared by all callers ───────
...
module.exports = { getPulsitVehicles, mapVehicle };
