...
// NOTE: This MUST stay above /:id or Express will treat
//       "stats" as a load ID and return 404
// ============================================================
...
// NOTE: This MUST stay above /:id — same reason as above
// ============================================================
...
router.get('/:id/comments', async (req, res) => {
  const { data, error } = await supabase
    ...
    .order('c_time', { ascending: true });   // oldest first for timeline
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
// ============================================================
// POST /api/loads — create new load with auto-generated number
// ============================================================
router.post('/', async (req, res) => {
  try {
    ...
    await supabase.from('lp_comments').insert([{
      ...
      c_comment:   `Load created. Truck: ${load.m_truck}, Customer: ${load.m_customer}, Route: ${load.m_from} → ${load.m_to}, Rate: R ${Number(load.m_rate || 0).toLocaleString('en-ZA')}`,
...
router.patch('/:id', async (req, res) => {
  ...
  if (req.body.m_status) {
    await supabase.from('lp_comments').insert([{
      ...
      c_comment:   `Status changed to ${req.body.m_status}`,
      c_logged_by: req.user.username,
    }]);
  }
...
// ============================================================
// DELETE /api/loads/:id — soft delete (ADMIN/MANAGER only)
// ============================================================
...
router.post('/:id/request-order-no', async (req, res) => {
  ...
  const { data: load } = await supabase
    .from('lp_movement')
    .select('m_order_no, m_order_no_pending')
    .eq('m_load_no', req.params.id)
    .single();

  if (!load) return res.status(404).json({ error: 'Load not found' });

  // If no existing order number, save directly without approval
