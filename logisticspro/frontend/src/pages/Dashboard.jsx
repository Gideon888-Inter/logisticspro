    ...
    <div style={{display:'flex', flexDirection:'column', gap:8}}>
      {data.map((d,i) => (
        <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
          ...
          <div style={{flex:1, background:'#f0f0f0', borderRadius:4, overflow:'hidden', height:24}}>// v2.1.0
import { useState, useEffect } from 'react';
...
function ServiceBanner({ vehicles, onNavigate }) {
  ...
  const dueVehicles = vehicles
    .filter(v => {
      const svcRem = (Number(v.vh_next_service)||0) - (Number(v.vh_odometer)||0);
      const whlRem = (Number(v.vh_next_wheel)||0) - (Number(v.vh_odometer)||0);
      return (v.vh_next_service && svcRem <= WARN_KM) || (v.vh_next_wheel && whlRem <= WARN_KM);
    })
    .map(v => ({
      ...v,
      _svcRem: (Number(v.vh_next_service)||0) - (Number(v.vh_odometer)||0),
      _whlRem: (Number(v.vh_next_wheel)||0) - (Number(v.vh_odometer)||0),
    }))
    .sort((a,b) => Math.min(a._svcRem, a._whlRem) - Math.min(b._svcRem, b._whlRem));

  if (dueVehicles.length === 0) return null;

  const overdue = dueVehicles.filter(v => v._svcRem <= 0 || v._whlRem <= 0);
  const bannerColor = overdue.length > 0 ? '#e53e3e' : '#f97316';
  const bgColor     = overdue.length > 0 ? '#fff1f2' : '#fff7ed';
  const borderColor = overdue.length > 0 ? '#e53e3e' : '#f97316';

  return (
    <div style={{ background: bgColor, border: `2px solid ${borderColor}`, borderRadius: 8, marginBottom: 4, overflow: 'hidden' }}>
      <div onClick={() => setExpanded(e => !e)} style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '12px 16px', cursor: 'pointer', background: bannerColor,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 20 }}>🔧</span>
          <span style={{ color: 'white', fontWeight: 700, fontSize: 14 }}>
            ...
            {dueVehicles.length - overdue.length > 0
              ? `${dueVehicles.length - overdue.length} vehicle${dueVehicles.length - overdue.length > 1 ? 's' : ''} due for service/alignment within 5,000 km`
              : ''}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={e => { e.stopPropagation(); onNavigate && onNavigate('vehicles'); }}
            style={{ color: 'white', fontSize: 12, textDecoration: 'underline', opacity: 0.9, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            View fleet →
          </button>
          <span style={{ color: 'white', fontSize: 18, fontWeight: 700 }}>{expanded ? '▲' : '▼'}</span>
      ...
      {expanded && (
        <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          {dueVehicles.map(v => {
            const svcOverdue = v._svcRem <= 0;
            const whlOverdue = v._whlRem <= 0;
            return (
              <div key={v.vh_code} style={{
                display: 'grid', gridTemplateColumns: '120px 100px 1fr 1fr',
                alignItems: 'center', gap: 12,
                background: (svcOverdue || whlOverdue) ? '#fee2e2' : '#fff7ed',
                borderRadius: 4, padding: '6px 12px',
                border: `1px solid ${(svcOverdue || whlOverdue) ? '#fca5a5' : '#fed7aa'}`,
              }}>
                <span style={{ fontWeight: 700, fontFamily: 'monospace', fontSize: 13 }}>{v.vh_code}</span>
                <span style={{ fontSize: 12, color: '#555' }}>{v.vh_type}</span>
                {v.vh_next_service ? (
                  <span style={{ fontSize: 12, fontWeight: svcOverdue ? 700 : 400, color: svcOverdue ? '#e53e3e' : '#c05621' }}>
                    Service: {svcOverdue ? `${Math.abs(v._svcRem).toLocaleString()} km overdue` : `${v._svcRem.toLocaleString()} km remaining`}
                  </span>
                ) : <span />}
                {v.vh_next_wheel ? (
                  <span style={{ fontSize: 12, fontWeight: whlOverdue ? 700 : 400, color: whlOverdue ? '#e53e3e' : '#c05621' }}>
                    Alignment: {whlOverdue ? `${Math.abs(v._whlRem).toLocaleString()} km overdue` : `${v._whlRem.toLocaleString()} km remaining`}
                  </span>
                ) : <span />}
              </div>
            );
